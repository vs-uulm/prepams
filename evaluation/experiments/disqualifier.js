...
module.exports = Array(13).fill(null).map((_, i) => ({
  name: `disqualifier-${i}`,
  DIR: `${__dirname}/../results/disqualifier-${i}`,
  ...
  generateWorkload(bar, config) {
    ...
    const dummies = Array(config.PARTICIPANTS).fill(null).map((_, i) => {
      bar.increment();
      const id = `dummy-${i}@example.org`;
      return {
        id: id,
    ...
    study.disqualifier = new Set(studies);
    study.numDisqualifiers = studies.length;

    // set participations
    const participations = [
      ...dummies.flatMap((participant) => studies.map((study) => {
